...
import { MemberFilterProvider } from "./member-filter-context"
...
export function TripSidebarLayout({
  ...
  const { allOnlineUserIds } = usePresence(trip.id)
  ...
  return (
    <MemberFilterProvider>
      <MemberPanel
        ...
        onInvite={() => { setIsPanelOpen(false); setShareOpen(true) }}
      />
      ...
      <div className="flex h-svh overflow-hidden">
        ...
        <aside className="hidden md:flex flex-col w-[30%] max-w-xs overflow-y-auto flex-shrink-0 bg-[#F7F3EE]" style={{ borderRight: "0.5px solid #D4C9BC" }}>
          ...
          <div className="px-4 py-3 space-y-2" style={{ borderBottom: "0.5px solid #D4C9BC" }}>
            ...
            {/* Member avatars — clickable to open panel */}
            {members.length > 0 && (
              <button
                type="button"
                onClick={() => setIsPanelOpen(true)}
                className="flex w-full items-center justify-between pt-1 hover:opacity-80 transition-opacity"
                title="View trip members"
              >
                <div className="flex -space-x-1.5">
                  {members.slice(0, 4).map((m) => (
                    <div key={m.userId} className="relative shrink-0">
                      <Avatar className="h-7 w-7 border-2 border-[#F7F3EE]" title={m.name}>
                      ...
                      {allOnlineUserIds.has(m.userId) && (
                        <span className="absolute bottom-0 right-0 h-2 w-2 rounded-full bg-[#22C55E] ring-[1.5px] ring-[#F7F3EE]" />
                      )}
                    </div>
                  ))}
                </div>
